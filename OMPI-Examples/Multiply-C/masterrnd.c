/*
  ...
  Jan 2002; Kees Lemmens; TWA-DIAM TUDelft.
  Last mod. Oct 2012
*/  
...
int master(long dim)
{
   ...
   for(x=0;x<nrprocs-1;x++)
   {
      ...
      MPI_Recv(&y,1,MPI_INT,MPI_ANY_SOURCE,12345,MPI_COMM_WORLD,&s);
      fprintf(stderr,"Receiving results in random order: now from %d ...\n",y);
      
      // And then get the actual results :
      MPI_Recv(&b[(y-1)*m],m,MPI_DOUBLE,y,y,MPI_COMM_WORLD,&s);
   }
